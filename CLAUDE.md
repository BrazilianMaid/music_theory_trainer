...
- **Tests**: Vitest, pure-function tests in `lib/__tests__/`. Run with `npm test`.
...
app/
  ...
  page.tsx            ← ErrorBoundary wrapper around dynamic AppShell (ssr:false)
...
components/
  ...
  ErrorBoundary.tsx   ← class-component error boundary, wraps AppShell
...
lib/
  ...
  adaptive.ts         ← Leitner-box state, recordResult, weakModules, moduleTrend, schema migrator
  theme.ts            ← useTheme hook (localStorage key: mtt_theme)
  instrument.ts       ← useInstrument hook + same-tab pub/sub (key: mtt_instrument)
  error-reporting.ts  ← reportError() — no-op stub, integration point for Sentry
  modules/            ← one file per quiz type, each exports a QuizModule
  __tests__/          ← Vitest pure-function tests (run with `npm test`)
```
...
- **Schema migration**: `loadState()` reads `mtt_adaptive_v{CURRENT_VERSION}` first; if absent, looks for older versions and runs `migrate()` to forward-port the data. To bump the schema, add a case to `migrate()` and increment `CURRENT_VERSION`.

### Error reporting

`lib/error-reporting.ts` exposes `reportError(error, context)`. Currently a console log in dev and a no-op in prod. The component-level `ErrorBoundary` calls it on caught errors. To wire up Sentry: install `@sentry/nextjs`, init in `instrumentation.ts` with the DSN, replace the body of `reportError` with `Sentry.captureException`, and add Sentry's ingest origin to `connect-src` in `vercel.json`'s CSP (currently `'self'` only).
...
|---|---|---|---|
| `mtt_adaptive_v{N}` | `lib/adaptive.ts` | `AdaptiveState` (see file) | bump `CURRENT_VERSION` + add a `migrate()` case |
...
- **No new dependencies without explicit approval.** Keep the dep tree small.
...
- **Use `reportError()` for unexpected failures** — not raw `console.error`. It gives a single pivot point when observability is wired up later.
- **Types in `lib/types.ts`.** Don't redeclare `Question`/`QuizModule`/`Instrument` locally.
- **Tests for engine math.** Add a Vitest case in `lib/__tests__/` for any new logic in `adaptive.ts` or `quiz-engine.ts`. Pure-function tests only; UI / hook tests are a future addition.
...
1. Increment `CURRENT_VERSION` in `lib/adaptive.ts` (the storage key is `mtt_adaptive_v{N}`).
2. Add a `case` in `migrate(parsed, fromVersion)` that takes the prior shape and returns the new one.
3. The migrator runs once on first load post-upgrade; new data is written under the new key.
4. **Update the storage table in this file** and confirm tests still pass with the new shape.
...
- **Tier-1 production hardening** — ErrorBoundary at root, `reportError()` integration point, Vitest test suite covering the adaptive engine, schema-migration scaffold in `loadState()`.
...
- **No observability service wired.** `reportError()` is a stub — point it at Sentry (or similar) before paid launch.
- **Sentry CSP gap.** CSP `connect-src 'self'` will block Sentry ingest. When wiring Sentry, also update `vercel.json` to include the Sentry ingest domain.
- **No UI tests.** Pure-function adaptive math is covered; component / hook behavior is verified manually.
