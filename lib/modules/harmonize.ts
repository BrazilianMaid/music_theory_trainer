...
const GUITAR_TIPS: Record<string, string> = {
  I: `In any major key, the root note of the I chord is the name of the key itself. G major → I = G. Strum I, then anything else, then back to I — feel the "home" pull. That sensation of arrival is tonic function in action.`,
  ii: `In C major, ii is Dm (D–F–A). Try Dm → G → C and listen — that's "ii–V–I", the engine of "Autumn Leaves," "Fly Me to the Moon," and a thousand pop tunes you've heard. On guitar, ii is always two letter-names up from I (C → D, G → A, D → E).`,
  iii: `In G major, iii is Bm. The progression G → Bm → C → D walks the bass G → B → C → D (almost stepwise) and sounds richer than just G → C → D. You'll hear this in folk and singer-songwriter music constantly — it's a quiet way to add emotional weight without changing key.`,
  IV: `In A major, IV is D. The IV chord is always two letter-names up from the I (count: A–B–C–D). I–IV–V in A is A–D–E — the spine of countless blues, country, and rock songs. If you only learn one progression to play in every key, learn this one.`,
  V: `In D major, V is A. Strum A, then immediately D — feel the lock. That's why every clockwise step on the Circle of Fifths <em>is</em> a V→I relationship: G is the V of C, D is the V of G, A is the V of D. The circle is literally a map of dominant relationships.`,
  vi: `In G major, vi is Em. Loop Em a few bars and the song feels minor; loop G and it feels major — same notes available either way. On guitar, the relative minor of any major key is the chord rooted three frets <em>down</em> from the major's root note (or equivalently, a minor 3rd below).`,
  'vii°': `In C major, vii° is B° (B–D–F). Try B° → C — feel the squeeze of the half-step from B up to C in the bass. On guitar you'll almost always play V7 instead (G7 in C major), but recognizing vii° as "the top of V7" unlocks chord-substitution thinking.`,
  i: `In A natural minor, i is Am (A–C–E). Try noodling on the A natural minor scale (A B C D E F G) and resolving every phrase to A — it sounds minor. Now resolve every phrase to C instead — same notes, suddenly major. That switching is what relative-key thinking unlocks.`,
  'ii°': `In A minor, ii° is B° (B–D–F). And in C major, vii° is also B°. Same chord, two different harmonic functions depending on where you call "home." This is one of the deepest insights in tonal music: chords don't have functions on their own — only in relation to the tonic.`,
  III: `In E natural minor, III is G major. Try Em → G → D → Em — the move from i up to III feels like stepping out into sunlight, and coming back down feels like returning indoors. On guitar, III is always three frets <em>up</em> from i (e.g. A minor's root at fret 5 → C major's root at fret 8 on the same string).`,
  iv: `In A minor, iv is Dm (D–F–A). On guitar, iv is the same shape as a major-key IV but with a flattened 3rd (one fret lower). If you can finger D major, you can finger Dm by sliding the F# down a fret to F. That single half-step is the entire mood shift between major and minor.`,
  v: `In A natural minor, v is Em (E–G–B). Try Em → Am: it sounds modal and ambiguous, like the song could drift anywhere. Now try E (or E7, with the raised G#) → Am: that G# is the leading tone, and you'll feel it yank the ear back to A. The choice between v and V is the choice between "modal" and "tonal" minor — and both are valid musical languages.`,
  ...
  VII: `In A natural minor, VII is G major. Strum Am → G → Am → G — that's the modal vamp under "Smoke on the Water"-style riffs and the entire vibe of Mixolydian-flavored rock. The b7 root gives you a power-chord move that descends naturally, no leading tone required.`,
}

const PIANO_TIPS: Record<string, string> = {
  I: `On piano, every I chord uses scale degrees 1–3–5 of the major scale stacked. In C major: C–E–G (all white keys). In G major: G–B–D. The shape changes (which black keys are involved) but the scale-degree pattern is universal — find degrees 1, 3, 5 of any major scale and you have the I chord.`,
  ii: `On piano, ii in C major is Dm (D–F–A) — three white keys, with the minor 3rd from D to F doing the harmonic work. The hand barely moves between ii and IV (Dm → F shares F and A): that overlap is why ii → V → I is the smoother cousin of IV → V → I in jazz comping.`,
  iii: `On piano, iii in C is Em (E–G–B) — and it shares two notes (E and G) with the C chord. Slide one finger up two scale degrees from I and you've landed on iii. That close kinship to the tonic is exactly why songs use iii as a passing chord between I and IV.`,
  IV: `On piano, IV is one position counter-clockwise from I on the Circle of Fifths. In C major: F–A–C. The shared C ties IV to I, and the F → E motion (when you go IV → I) gives the gentle "amen" resolution. I–IV–V–I in C uses just the white keys — it's the natural three-chord box.`,
  V: `On piano, V → I has two half-step motions hidden inside it: the leading tone (degree 7 of the key) rises a half-step to tonic, and the 4th of the key (when V becomes V7) falls a half-step to the 3rd of I. In C major: B → C and F → E. Those two half-steps are the engine of dominant resolution.`,
  vi: `On piano, vi shares two notes with the I chord. In C major, vi is Am (A–C–E), overlapping with C–E–G by two notes. This makes vi the most natural detour from home — a one-finger move that completely changes mood from major to relative minor without forcing your hand to relearn anything.`,
  'vii°': `On piano, vii° in C is B° (B–D–F) — three consecutive thirds, all on white keys. It's also the upper structure of V7 (G7 = G–B–D–F): the top three notes B–D–F <em>are</em> the vii° triad. Recognizing this overlap is the key to chord-substitution thinking on the keyboard.`,
  i: `On piano, the i chord in A minor is A–C–E — exactly the same notes as the vi chord of C major. Same chord, different home base. Toggle your phrases to land on A vs. C and the entire piece switches between modal centers without changing a single key under your fingers.`,
  'ii°': `On piano, ii° in A minor is B° (B–D–F) — and vii° in C major is also B°. Same three keys, two different harmonic functions depending on which note you treat as "home." This is one of the deepest insights of tonal music: chords don't have functions on their own, only in relation to the tonic.`,
  III: `On piano, III in A minor is C major (C–E–G) — the relative major's tonic, sitting inside the minor key. Use it as a brightness lift: i → III (Am → C) feels like stepping into sunlight without leaving the minor key. Two of the three notes (C, E) are shared between Am and C, so the hand barely shifts.`,
  iv: `On piano, iv in A minor is Dm (D–F–A). Compare it to A major's IV (D major: D–F#–A): the only difference is one key (F vs. F#). That single half-step — one black key vs. a white key — is the entire mood difference between IV-in-major and iv-in-minor.`,
  v: `On piano, v in A natural minor is Em (E–G–B) — three white keys. It lacks the leading tone (G#), so it doesn't pull strongly back to A. Raising G to G# (a black key) converts v (Em) to V (E major: E–G#–B), giving you a real cadence. That single black-key substitution is the entire move from "modal minor" to "tonal minor."`,
  VI: `On piano, VI in A minor is F major (F–A–C). It shares two notes with i (Am uses A–C–E). The move i → VI (Am → F) is the bass dropping a major 3rd while two of the three notes stay put — a one-finger shift that adds a major-key brightness without abandoning the minor home.`,
  VII: `On piano, VII in A minor is G major (G–B–D) — a whole step below the tonic. The "rock bVII" feel comes from the lack of leading-tone tension: VII → i isn't a half-step pull like V → i would be, but instead a gentle whole-step descent that sounds modal rather than tonal.`,
}
